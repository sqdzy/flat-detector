...
|---|---|---|---|
| Schema + Alembic | Initial models and connection-injectable migration | SQLite upgrade/current/down | Live PostgreSQL integration test included in GitHub CI but not yet executed; backup/restore |
...
| Delivery | Timezone scheduler, durable outbox; 429 retry; crash-recovery marks stale CLAIMED → UNCERTAIN | Mock SQLite and crash-recovery regression passed | PostgreSQL lock/concurrency test added to CI (pending live run); rate limiting for larger audiences |
...
**GitHub status:** `sqdzy/flat-detector` is empty as last verified; working source and documentation are in the local release archive. New `postgres:17-alpine` GitHub Actions integration test is **prepared but not executed**, since files have **not** been pushed or deployed by this run.
