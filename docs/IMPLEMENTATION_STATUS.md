...
## 2026-09-24 — scraper-first technical probe
Per owner preference, the next source experiment is scraper-first and free. Added an isolated Avito Playwright/Chromium probe with a persistent profile, exact-host URL validation, current public data-marker extraction, challenge/rate-limit/structure state classification, and zero database writes. It intentionally contains no fingerprint modification, CAPTCHA solver, proxy rotation, cookie import, or automated anti-bot bypass. The browser container receives neither PostgreSQL nor Telegram secrets and is not attached to the internal application network. CI builds the separate browser image and runs an offline Chromium DOM smoke before any VPS live request. See [AVITO_BROWSER_PROBE.md](AVITO_BROWSER_PROBE.md). A successful live probe is required before database or Telegram wiring.
